 ...
 * @constructor
 */
var Client = function (connectionString, clientId, zkOptions, socketTimeout, logger) {
    if (this instanceof Client === false) {
        return new Client(connectionString, clientId, zkOptions,
                          socketTimeout);
    }
    ...
    this.socketTimeout = socketTimeout || 0;
    this.brokers = {};
...
Client.prototype.sendToBroker = function (payloads, encoder, decoder, cb) {
    ...
    for (var leader in payloads) {
        ...
        broker.write(request);
        if (!longpolling) {
            broker.setTimeout(this.socketTimeout);
        }
    }
};
...
Client.TimeoutError = function TimeoutError(socket, timeout) {
    Error.call(this);
    Error.captureStackTrace(this, this.constructor);
    this.name = this.constructor.name;
    this.socket = socket;
    this.message = util.format("Connection to %s (socketId %d) timed out after %dms", socket.addr, socket.socketId, timeout);
};
util.inherits(Client.TimeoutError, Error);
...
Client.prototype.createBroker = function connect(host, port, longpolling) {
    ...
    socket.on('timeout', function() {
        socket.destroy();
        self.emit('error', new Client.TimeoutError(socket, self.socketTimeout));
    });
    socket.on('close', function (had_error) {
        delete self.brokers[socket.addr];
        ...
        var message = had_error ? this.error.message : 'Broker not available';
        var error = new errors.BrokerNotAvailableError(message);
...
Client.prototype.unqueueCallback = function (broker, id) {
    ...
    var longpolling = _.all(queue, function(tuple) {
        var decoder = tuple[0];
        return decoder.name === 'decodeFetchResponse';
    });
    if (longpolling) {
        broker.setTimeout(0);
    }
