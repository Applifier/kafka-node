...
Client.prototype.updateMetadatas = function (metadatas) {
    ...
    this.logger(util.format("updateMetadatas with metadatas %j, current topicMetadata %j", metadatas, this.topicMetadata));
    ...
    for(var topic in this.topicMetadata) {
        ...
        this.logger(util.format("updateMetadatas updated topic %s partitions to %s", topic,
          this.topicPartitions[topic].join(", ")));
    }
};
...
Client.prototype.clearCallbackQueue = function (broker, error) {
    ...
    var queue = this.cbqueue[brokerId];
    this.logger(queue.length + " callbacks queued");
    if (!longpolling) {
        this.logger("Calling callbacks");
